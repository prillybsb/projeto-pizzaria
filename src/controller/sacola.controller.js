...
const findByIdSacolaController = async (req, res) => {
  try {
    return res
      .status(200)
      .send(await sacolaService.findByIdSacolaService(req.params.id));
  } catch (error) {
    console.log(`erro: ${error.message}`);
    return res
      .status(500)
...
const updateSacolaController = async (req, res) => {
  try {
    return res
      .status(202)
      .send(await sacolaService.updateSacolaService(req.params.id, req.body));
  } catch (error) {
    console.log(`erro: ${error.message}`);
    return res
      .status(500)
...
const deleteSacolaController = async (req, res) => {
  try {
    return res
      .status(200)
      .send(await sacolaService.deleteSacolaService(req.params.id));
  } catch (error) {
    console.log(`erro: ${error.message}`);
    return res
      .status(500)
